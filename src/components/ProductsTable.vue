...
<template>
    <h2 class="text-2xl font-medium mb-6 text-black">Products Table</h2>
    <div class="relative overflow-hidden rounded-lg w-auto p-4 mb-14 bg-gray-800">
      <table class="w-full text-sm text-left text-gray-400">
        <thead class="text-xs uppercase bg-gray-700 text-gray-400">
          <tr>
            ...
            <th scope="col" class="px-6 py-3 text-2xl">Image URL</th>
            <th scope="col" class="px-16 py-3 text-2xl">Action</th>
          </tr>
        </thead>
        <tbody>
          <tr v-for="(product, index) in products" :key="product.id" :class="index % 2 === 0 ? 'bg-gray-800' : 'bg-gray-700'">
            <td class="text-2xl px-6 py-4 font-medium text-white hover:bg-gray-600">{{ product.id }}</td>
            ...
            <img :src="product.imageurl" alt="Product Image" class="w-20 h-20 object-cover m-auto">
            </td>
            <td class="text-2xl px-6 py-8 hover:bg-gray-600 flex justify-between">
              <button @click="openModal" class="bg-emerald-400 hover:bg-emerald-500 text-black px-2 py-2 rounded-lg">Add Product</button>
      ...
      <div v-if="isModalOpen" class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
        <div class="bg-gray-800 p-4 rounded-lg shadow-lg w-full max-w-md">
          <button @click="closeModal" class="text-4xl float-right text-white mx-5 my-5">&times;</button>
