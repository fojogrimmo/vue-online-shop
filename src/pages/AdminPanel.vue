...
<template>
  ...
  <button @click="logout" class="bg-red-500 hover:bg-red-600 text-white px-4 py-2 rounded-lg mb-4">Log Out</button>
  ...
  <div class="relative overflow-x-auto w-auto p-4 mb-14 rounded-lg">
    <table class="w-full text-sm text-left text-gray-400 rounded-lg overflow-hidden">
      ...
      <tbody>
        <tr v-for="(user, index) in users" :key="user.id" :class="index % 2 === 0 ? 'bg-gray-800' : 'bg-gray-700'">
          ...
          <td class="text-2xl px-6 py-4 text-center hover:bg-gray-600">
            <input type="checkbox" :checked="user.is_admin" @change="toggleAdmin(user.id, $event.target.checked)" class="form-checkbox text-2xl">
          </td>
          <td class="text-2xl px-6 py-4 hover:bg-gray-600 flex justify-between ">
            <button @click="openModal" class="bg-emerald-400 hover:bg-emerald-500 text-black px-2 py-2 rounded-lg">Add User</button>
    ...
    <div v-if="isModalOpen" class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
      <div class="p-4 rounded-lg shadow-lg w-full max-w-md">
        <button @click="closeModal" class="float-right mx-8 my-8 text-white text-3xl">&times;</button>
